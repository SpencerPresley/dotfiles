-- BASIC SETTINGS (safe for both VS Code and terminal Neovim)
...
vim.opt.spelllang = { "en" } -- spellchecking language
...
-- Behavior Settings (safe for both)
...
-- Split Behavior (safe for both)
vim.opt.splitbelow = true -- Horizontal splits open below
vim.opt.splitright = true -- Vertical splits open to the right

-- Terminal Neovim only settings (these cause issues in VS Code/Cursor)
if not vim.g.vscode then
	vim.opt.number = true -- line numbers
	vim.opt.relativenumber = true -- relative line numbers
	vim.opt.cursorline = true -- highlight the current line
	vim.opt.cmdheight = 1 -- cmd line height
	vim.opt.grepprg = "rg --vimgrep" -- Ripgrep > Grep
	...
	vim.opt.laststatus = 3 -- one global statusline (no per-window truncation, e.g. the `<` marker)
	vim.opt.colorcolumn = "100" -- Show column at 100 chars
	vim.opt.showmatch = true -- Highlight matching brackets
	vim.opt.matchtime = 2 -- How long to show matching bracket
	vim.opt.completeopt = "menuone,noinsert,noselect" -- Completion ops
	...
	vim.opt.lazyredraw = false -- Redraw while executing macros (UX)
	...
	vim.opt.autochdir = false -- Don't change dir automatically
	...
	local undodir = "~/.local/share/nvim/undodir" -- Undo dir path
