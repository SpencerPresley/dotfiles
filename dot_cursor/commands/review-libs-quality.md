...
- **Consistency** across the three packages
...
1. **Read the directory structure** of each package in `libs/`:
   ...
   - `libs/atlas/` - Additional agents (tshark variant)
...
- Is the separation between `core`, `purplehaze`, and `atlas` clear and justified?
...
- **Docstring formatting**: There's a separate task for that
...
**Scope**: `libs/` directory and its three packages (`core`, `purplehaze`, `atlas`)
