---
description: LangChain/LangGraph code review with current pattern verification
allowed-tools: Read, Glob, Grep, WebSearch, mcp__docs-langchain__SearchDocsByLangChain, mcp__context7__resolve-library-id, mcp__context7__get-library-docs
argument-hint: <file path or @file reference>
---
...
- **docs-langchain MCP** (`SearchDocsByLangChain`): Search LangChain, LangGraph, and deep-agents documentation
...
- deepagents==0.3.0
- pydantic==2.12.5
- Python==3.10+ (3.12/3.13 features allowed)
...
Search docs-langchain MCP for current patterns before flagging issues:
...
**docs-langchain MCP (SearchDocsByLangChain):**

Run numerous parallel searches. The docs-langchain MCP allows you to search all documentation under the langchain namespace including:
...
- Ptyhon 3.10+ Use `|` as well but at top import `from __future__ import annotations`
